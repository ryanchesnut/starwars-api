...
// src/functions/skywalker-starships/handler.ts
var handler_exports = {};
...
// src/functions/skywalker-starships/controller.ts
var getLukesStarships = async () => {
  let lukeData = await NodeFetch.get(process.env.STARWARS_BASE_API, "/people/1");
  console.log(lukeData);
  return getStarships(lukeData);
};
var getStarships = async (lukeData) => {
  var _a4;
  if (!lukeData) {
    throw Error("No Luke Data");
  }
  const starshipsPromises = (_a4 = lukeData == null ? void 0 : lukeData.starships) == null ? void 0 : _a4.map((starshipUrl) => {
    return new Promise(async (resolve, reject) => {
      starshipUrl = starshipUrl.replace(process.env.STARWARS_BASE_API, "");
      const res = await NodeFetch.get(process.env.STARWARS_BASE_API, starshipUrl);
      if (!res) {
        return reject(null);
      }
      return resolve(res);
...
// src/functions/skywalker-starships/handler.ts
var handler = async () => {
  let data = await getLukesStarships();
