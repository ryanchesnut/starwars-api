...
// src/functions/species-classification/handler.ts
var handler_exports = {};
...
// src/functions/species-classification/controller.ts
var getSpeciesClassificationEp1 = async () => {
  let filmData = await NodeFetch.get(process.env.STARWARS_BASE_API, "/films/4/");
  console.log(filmData);
  return getSpecies(filmData);
};
var getSpecies = async (filmData) => {
  var _a4;
  if (!filmData) {
    throw Error("No Film Data");
  }
  const speciesPromises = (_a4 = filmData == null ? void 0 : filmData.species) == null ? void 0 : _a4.map((speciesUrl) => {
    return new Promise(async (resolve, reject) => {
      speciesUrl = speciesUrl.replace(process.env.STARWARS_BASE_API, "");
      const res = await NodeFetch.get(process.env.STARWARS_BASE_API, speciesUrl);
      if (!res) {
        return reject(null);
      }
...
// src/functions/species-classification/handler.ts
var handler = async () => {
  let data = await getSpeciesClassificationEp1();
