...
// src/functions/galaxy-population/handler.ts
var handler_exports = {};
...
var middy = (lambdaHandler = defaultLambdaHandler, plugin = {}) => {
  ...
  (_a4 = plugin.beforePrefetch) == null ? void 0 : _a4.call(plugin);
  const beforeMiddlewares = [];
  const afterMiddlewares = [];
  const onErrorMiddlewares = [];
  const middy2 = (event = {}, context = {}) => {
    var _a5;
    (_a5 = plugin.requestStart) == null ? void 0 : _a5.call(plugin);
    const request = {
      event,
      context,
      response: void 0,
...
var runRequest = async (request, beforeMiddlewares, lambdaHandler, afterMiddlewares, onErrorMiddlewares, plugin) => {
  ...
  try {
    ...
    if (typeof request.response === "undefined") {
      (_a4 = plugin.beforeHandler) == null ? void 0 : _a4.call(plugin);
...
var runMiddlewares = async (request, middlewares, plugin) => {
  ...
  for (const nextMiddleware of middlewares) {
    (_a4 = plugin.beforeMiddleware) == null ? void 0 : _a4.call(plugin, nextMiddleware.name);
...
// src/functions/galaxy-population/controller.ts
var getGalaxyPopulation = async () => {
  let numberOfPlanets = await getNumberOfPlanets();
  const allPlanetsData = await getAllPlanets(numberOfPlanets);
  return sumPlanetPopulation(allPlanetsData);
};
var getNumberOfPlanets = async () => {
  let planetData = await NodeFetch.get(process.env.STARWARS_BASE_API, "/planets");
  return planetData == null ? void 0 : planetData.count;
};
var getAllPlanets = async (numberOfPlanets) => {
  if (!numberOfPlanets) {
    throw Error("Must have a number of planets");
  }
  let planetPromiseArray = [];
  for (let i2 = 1; i2 <= numberOfPlanets; i2++) {
    planetPromiseArray.push(
      new Promise(async (resolve, reject) => {
        const res = await NodeFetch.get(process.env.STARWARS_BASE_API, `/planets/${i2}`);
        if (!res) {
          return reject(null);
        }
        return resolve(res);
...
var sumPlanetPopulation = (planetData) => {
  const initialValue = 0;
  let total = planetData.reduce((acc, planet) => {
...
// src/functions/galaxy-population/handler.ts
var handler = async () => {
  let data = await getGalaxyPopulation();
